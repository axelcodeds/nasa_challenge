  ...
  function updateWeatherData(weatherData) {
    ...
    document.getElementById(
      ...
      let cloudIntensity;
  if (airQuality <= 50) {
    cloudIntensity = 'despejado';
  } else if (airQuality <= 100) {
    cloudIntensity = 'nublado';
  } else {
    cloudIntensity = 'tormenta';
  }

  // Llamar a la función de nubes
  updateCloudIntensity(cloudIntensity);
    updateSunAnimation(uvIndex);
    updateAirQualityBackground(airQuality);
    updateWindAnimation(airQuality);
    
  }
// Variables globales para las nubes
let cloudInterval;

// Función para obtener el contenedor de nubes de forma segura
...
function createCloud(type) {
  ...
  // Velocidad aleatoria basada en el tipo
  let speed;
  switch(type) {
    case 1: speed = Math.random() * 60 + 40; break;
    case 2: speed = Math.random() * 80 + 60; break;  
    case 3: speed = Math.random() * 100 + 80; break;
  }
  
  cloud.style.animationDuration = `${speed}s`;
  cloud.style.animationDelay = `${Math.random() * 20}s`;
  cloud.style.opacity = (Math.random() * 0.3 + 0.5).toString();
...
// Función para limpiar todas las nubes
...
// Función principal para actualizar la intensidad de nubes
function updateCloudIntensity(intensity) {
  const cloudContainer = getCloudContainer();
  if (!cloudContainer) {
    console.warn('Contenedor de nubes no encontrado');
    return;
  }
  
  clearClouds();
  
  let cloudCount, intervalTime, types;
  ...
  for (let i = 0; i < cloudCount; i++) {
    ...
    createCloud(randomType);
  }
  
  // Configurar intervalo para crear nuevas nubes
...
setTimeout(() => {
  updateWeatherData({ uvIndex: 5, airQuality: 75 });
}, 100);
